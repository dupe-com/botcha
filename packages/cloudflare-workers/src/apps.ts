...
export interface AppConfig {
  ...
  registration_policy?: 'open' | 'paired'; // Default 'open' (backward-compat)
}
...
export type PublicAppConfig = {
  ...
  email: string;
  email_verified: boolean;
  registration_policy: 'open' | 'paired';
};
...
export async function getApp(
  ...
  try {
    ...
    return {
      ...
      email: config.email,
      email_verified: config.email_verified,
      registration_policy: config.registration_policy ?? 'open',
    };
  } catch (error) {
...
/**
 * Update the registration_policy for an app.
 * 'open'   — anyone with a valid app_id token can register agents (default, backward-compat)
 * 'paired' — a one-time pair_code generated by the app owner is required at registration time
 */
export async function updateAppRegistrationPolicy(
  kv: KVNamespace,
  app_id: string,
  policy: 'open' | 'paired'
): Promise<boolean> {
  try {
    ...
    console.error(`Failed to update registration policy for ${app_id}:`, error);
    return false;
  }
}
...
const PAIR_CODE_ALPHABET = '23456789ABCDEFGHJKMNPQRSTUVWXYZ'; // no 0,O,1,I,L
const PAIR_CODE_TTL_SEC = 15 * 60; // 15 minutes

function generatePairCodeToken(): string {
  const bytes = new Uint8Array(6);
  crypto.getRandomValues(bytes);
  const chars = Array.from(bytes).map(b => PAIR_CODE_ALPHABET[b % PAIR_CODE_ALPHABET.length]).join('');
  return `PAIR-${chars}`;
}

/**
 * Generate a short-lived, single-use pair code for agent registration.
 * Stored in the CHALLENGES KV namespace as `pair_code:{code}`.
...
export async function createPairCode(
  ...
  const code = generatePairCodeToken();
  await challengesKv.put(
    `pair_code:${code}`,
    JSON.stringify({ app_id, created_at: Date.now(), used: false }),
...
/**
 * Validate and consume a pair code for agent registration.
 * Returns true if the code was valid for the given app and has now been consumed.
 * Returns false if the code doesn't exist, is expired, was already used, or belongs to a different app.
 */
export async function consumePairCode(
  ...
  try {
    const raw = await challengesKv.get(`pair_code:${code}`, 'text');
    if (!raw) return false;
    const data = JSON.parse(raw) as { app_id: string; created_at: number; used: boolean };
    if (data.used || data.app_id !== expected_app_id) return false;
    // Mark consumed (short remaining TTL)
    data.used = true;
    await challengesKv.put(`pair_code:${code}`, JSON.stringify(data), { expirationTtl: 60 });
    return true;
  } catch {
    return false;
  }
